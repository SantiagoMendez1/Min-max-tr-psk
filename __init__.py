...
class DataAnalyzer:
    ...
    def model_hyperparam_tuning(self, run=False):
        ...
        Returns:
        products_to_train: A list of the filtered products used for training.
        """
    ...
    def prediction(self, product, months_to_predict):
        ...
        best_params = self.all_best_params[str(product)]
        
        X_train = self.train[['week', 'year']]
        y_train = self.train[product]

        X_test = self.test[['week', 'year']]
        y_test = self.test[product]
    ...
    def plot_test(self, product):
        ...
        try:
            ...
            ax.legend()
            ax.set_title(f'Prediction for test data of the {product}')
            ax.grid(True)
            plt.tight_layout()

            try:
                plt.savefig(f'{self.USER_PATH}/model/rf_prediction_{str(product)}_test.png')
            except IOError as e:
    ...
    def plot_months_to_predict(self, product):
        """
        Plot the predicted quantities for a given number of months with a 95% confidence interval.

        Args:
        product (int): The identifier of the product for which the predictions are plotted.
        months_to_predict (int): The number of months for which predictions are made.
        """
        try:
            fig, ax = plt.subplots(figsize=(15, 6))
            ...
            plt.xticks(rotation=45)
            ax.legend()
            ax.set_title(f'Prediction for the {product}')
            ax.grid(True)
            plt.tight_layout()

            try:
                plt.savefig(f'{self.USER_PATH}/model/rf_prediction_{str(product)}_{str(self.months_to_predict)}_months.png')
            except IOError as e:
                print(f"Error saving plot: {e}")
...
"""
    def plot_months_to_predict(self, product, months_to_predict):
        ...
        for i, (date, pred) in enumerate(zip(self.y_pred.index, self.df_pred_monthly['Quantity Prediction'])):
            ax.annotate(f'{pred:.2f}', 
                        xy=(date, pred), 
                        xytext=(3, 3), 
                        textcoords='offset points', 
        ...
        for i, (date, lower) in enumerate(zip(self.y_pred.index, self.df_pred_monthly['Lower Quantity'])):
            ax.annotate(f'{lower:.2f}', 
                        xy=(date, lower), 
                        xytext=(3, -15), 
                        textcoords='offset points', 
        ...
        for i, (date, upper) in enumerate(zip(self.y_pred.index, self.df_pred_monthly['Higher Quantity'])):
            ax.annotate(f'{upper:.2f}', 
                        xy=(date, upper), 
                        xytext=(3, 15), 
                        textcoords='offset points', 
        ...
        ax.grid(True)
        plt.tight_layout()
        plt.savefig(f'{self.USER_PATH}/model/rf_prediction_{str(product)}_{str(months_to_predict)}_months.png')
"""